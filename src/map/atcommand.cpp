...
ACMD_FUNC(partyrecall)
{
	...
	sprintf(atcmd_output, msg_txt(sd,95), p->party.name); // // Todos los miembros online de la Party %s han sido llamados hasta tu ubicacion.
	clif_displaymessage(fd, atcmd_output);
	if (count) {
		sprintf(atcmd_output, msg_txt(sd,1033), count); // Because you are not authorized to warp from some maps, %d player(s) have not been recalled.
		clif_displaymessage(fd, atcmd_output);
...
{ // [Vykimo]
	...
	if( !sd->status.party_id ) {
		clif_displaymessage(sd->fd, "You're not in a party.");// You're not in a party.
		return -1;
	}
	...
	if( sd->state.spb ) {
		sd->state.spb = 0;
		clif_displaymessage(sd->fd, "Displaying party member's buffs disabled."); // Displaying party member's buffs disabled.
	} else {
		sd->state.spb = 1;
		clif_displaymessage(sd->fd, "Displaying party member's buffs enabled."); // Displaying party member's buffs enabled.
	}
...
#include "../custom/atcommand_def.inc"
		//xRO
		ACMD_DEF(partybuff), // [EasyDev]
		ACMD_DEF2("spb", partybuff), // [EasyDev]

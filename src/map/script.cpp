...
static bool script_rid2sd_( struct script_state *st, struct map_session_data** sd, const char *func ){
 ...
 /// Returns the player attached to this script, identified by the rid.
 /// If there is no player attached, the script is terminated.
 TBL_PC *script_rid2sd_verify(struct script_state *st)
 { 
...
/*==========================================
 * EasyDev Simple funcion mes que presenta 
 * el boton close de una vez.
 *------------------------------------------*/

BUILDIN_FUNC(mclose) 
{
	TBL_PC* sd = script_rid2sd_verify(st);
	if( sd == NULL )
		return SCRIPT_CMD_SUCCESS;
...
/*==========================================
 *
 * [EasyDev] Adaptacin para xRO
 * Nombre De Npc directo
 *
*------------------------------------------*/
BUILDIN_FUNC(title) 
{
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(seguridad,"i"),
	BUILDIN_DEF(verseguridad,""),
